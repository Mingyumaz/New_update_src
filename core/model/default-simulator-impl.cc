...
DefaultSimulatorImpl::ProcessOneEvent (void)
{
  ...
  NS_ASSERT (next.key.m_ts >= m_currentTs); // @mingyu-with-bmv2
  m_unscheduledEvents--;
...
DefaultSimulatorImpl::Schedule (Time const &delay, EventImpl *event)
{
  ...
  NS_ASSERT_MSG (SystemThread::Equals (m_main), "Simulator::Schedule Thread-unsafe invocation!"); //@mingyu-with-bmv2

  Time tAbsolute = delay + TimeStep (m_currentTs);

  NS_ASSERT (tAbsolute.IsPositive ());
  NS_ASSERT (tAbsolute >= TimeStep (m_currentTs)); //@mingyu-with-bmv2
  Scheduler::Event ev;
  ev.impl = event;
