...
#ifndef ONOFF_PRO_APPLICATION_H
#define ONOFF_PRO_APPLICATION_H
...
class OnOffProApplication : public Application 
{
public:
  /**
  ...
  OnOffProApplication ();
  OnOffProApplication (DataRate m_cbrRate, uint32_t m_pktSize, Address m_peer,
                        Ptr<RandomVariableStream> m_onTime,
                        Ptr<RandomVariableStream> m_offTime,
                        TypeId m_tid);

  virtual ~OnOffProApplication();
  ...
  */
  ...
  /**
   * @brief set the data rate for sending
   * 
   * @param rate 
   */
